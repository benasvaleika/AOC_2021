...
// Part 2 not my solution, didn't have a lot of time, and wanted to try other excercises as i was stuck on this one

const sum = (xs: number[]): number => xs.reduce((sum, x) => sum + x, 0);
...
const decodeOutputs = (mapper: string[][], outputs: string[][]): number => {
  const code = outputs.reduce(
    (code, chars) => code + decodeOutput(mapper, chars),
    ""
  );
...
const decodeOutput = (mapper: string[][], output: string[]): number =>
  mapper.findIndex((k) =>
    k.every((x) => output.includes(x) && output.every((x) => k.includes(x)))
  );
...
const input = readFileSync("./src/Day8/day8.txt", "utf-8").split("\n");
...
const p1 = sum(
  input
    ...
    .map(
      ([_, outputs]) =>
        outputs.filter((o) => [2, 3, 4, 7].includes(o.length)).length
    )
);
