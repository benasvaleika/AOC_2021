...
const powerRate = (
  report: string[],
  selector: (ones: number, zeros: number) => number
): number => {
...
const report = readFileSync("./src/Day3/day3-input.txt").toString().split("\n");
...
const oxygen = gasRate(
  report,
  (ones, zeros, expected) => (ones >= zeros ? "1" : "0") === expected
);
const carbon = gasRate(
  report,
  (ones, zeros, expected) => (zeros <= ones ? "0" : "1") === expected
);
